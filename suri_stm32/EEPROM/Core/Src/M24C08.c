...
void M24C08_Write (uint16_t page, uint16_t offset, uint8_t *data, uint16_t size)
{
	...
	for (int i=0; i<numofpages; i++)
	{
		...
		HAL_I2C_Mem_Write(&hi2c1, M24C08_ADDR, MemAddress, 2, &data[pos], bytesremaining, 1000);  // write the data to the EEPROM
...
void M24C08_Write_NUM (uint16_t page, uint16_t offset, float data)
{

	float2Bytes(bytes_temp, data);

	M24C08_Write(page, offset, bytes_temp, 4);
}
...
float M24C08_Read_NUM (uint16_t page, uint16_t offset)
{
	...
	M24C08_Read(page, offset, buffer, 4);

	return (Bytes2float(buffer));
}

/* READ the data from the EEPROM
 * @page is the number of the start page. Range from 0 to 63
 * @offset is the start byte offset in the page. Range from 0 to 15
 * @data is the pointer to the data to write in bytes
 * @size is the size of the data
 */
void M24C08_Read (uint16_t page, uint16_t offset, uint8_t *data, uint16_t size)
{
	...
	for (int i=0; i<numofpages; i++)
	{
		...
		HAL_I2C_Mem_Read(&hi2c1, M24C08_ADDR, MemAddress, 2, &data[pos], bytesremaining, 1000);
		startPage += 1;
		offset=0;
...
void M24C08_PageErase (uint16_t page)
{
	...
	// write the data to the EEPROM
	HAL_I2C_Mem_Write(&hi2c1, M24C08_ADDR, MemAddress, 2, data, PAGE_SIZE, 1000);
