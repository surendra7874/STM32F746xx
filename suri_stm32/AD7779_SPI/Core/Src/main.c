...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
//uint16_t  modifyValue;   //Value of the register after modification
...
void readRegister(uint8_t regAdd)
{
	  ...
	  HAL_GPIO_WritePin(ADS_CS_GPIO_Port, ADS_CS_Pin, GPIO_PIN_RESET);     //CS pin Low
	 HAL_SPI_Transmit(&hspi2, &readCommand, 1, 10);    // Receiving the RESET value of the register
	 HAL_SPI_Receive(&hspi2, &receiveData, 2, 10);
	 HAL_GPIO_WritePin(ADS_CS_GPIO_Port, ADS_CS_Pin, GPIO_PIN_SET);   // CS pin High
	  //return receiveData;
}
